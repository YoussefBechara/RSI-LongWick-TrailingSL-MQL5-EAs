...
void OnTick(){
   ...
   if (currentCandleTime != lastCandleTime)
   {
      ...
      double past_upwick_avg;
      double past_lowick_avg;
      ...
      for(int i=1;i<=candlebody_avg_lookback;i++){
         ...
         double curr_high = iHigh(NULL,PERIOD_CURRENT,i);
         double curr_low = iLow(NULL,PERIOD_CURRENT,i);
         
         sum_candlebodies = sum_candlebodies + MathAbs(curr_open-curr_close) ;
         if (curr_high-curr_close>curr_high-curr_open){sum_upwicks = sum_upwicks + (curr_high-curr_open); sum_lowicks = sum_lowicks + (curr_close-curr_low);} //red candle
         else if (curr_high-curr_close<=curr_high-curr_open){sum_upwicks = sum_upwicks + (curr_high-curr_close); sum_lowicks = sum_lowicks + (curr_open-curr_low);} //green candle
      }
      past_candlebody_avg = sum_candlebodies/candlebody_avg_lookback;
      past_upwick_avg = sum_upwicks/candlebody_avg_lookback;
      past_lowick_avg = sum_lowicks/candlebody_avg_lookback;
      ...
      double upwick_to_avg_ratio = upper_wick_length/past_upwick_avg;
      double lowick_to_avg_ratio = lower_wick_length/past_lowick_avg;
      bool buying_condition = false;
      bool selling_condition = false;
      
      if(upwick_body_ratio>=3 && lowick_body_ratio<=2 && body_to_avg_ratio>=0.3 && upwick_to_avg_ratio>=2){selling_condition=true;}
      else if(lowick_body_ratio>=3 && upwick_body_ratio<=2 && body_to_avg_ratio>=0.3 && lowick_to_avg_ratio>=2){buying_condition=true;}
